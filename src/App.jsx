import { m as motion, useMotionTemplate, useMotionValue, useScroll, useSpring } from 'framer-motion'
...
function ProjectCard({ project, index }) {
  return <motion.article {...reveal} className="group grid overflow-hidden rounded-2xl border border-line bg-panel transition duration-500 hover:border-zinc-500/60 lg:grid-cols-[.8fr_1.2fr]">
    ...
    <div className="relative min-h-[360px] overflow-hidden bg-zinc-950"><motion.img src={project.imageSmall || project.image} srcSet={project.imageSmall ? `${project.imageSmall} ${project.imageSmallWidth}w, ${project.image} ${project.imageWidth}w` : undefined} sizes="(min-width: 1280px) 660px, (min-width: 1024px) 52vw, 100vw" alt={`Interface do projeto ${project.title} para ${project.subtitle.toLowerCase()}`} loading="lazy" decoding="async" width={project.imageWidth} height={project.imageHeight} style={{ objectPosition: project.imagePosition }} className="absolute inset-0 h-full w-full object-cover transition duration-700 group-hover:scale-[1.04]" /><div className="absolute inset-0 bg-gradient-to-t from-black/55 via-transparent to-black/10" /><div className="absolute inset-x-5 top-5 flex items-center justify-between rounded-xl border border-white/15 bg-black/40 px-4 py-3 backdrop-blur-md"><div className="flex gap-1.5" aria-hidden="true"><i className="h-2 w-2 rounded-full bg-zinc-500" /><i className="h-2 w-2 rounded-full bg-zinc-500" /><i className="h-2 w-2 rounded-full bg-zinc-500" /></div><span className="text-[10px] font-medium uppercase tracking-[.16em] text-white/70">{project.status}</span></div></div>
  </motion.article>
}
